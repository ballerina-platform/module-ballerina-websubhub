...
# Server ID is is used to uniquely identify each server. Each server must have a unique ID.
...
# Kafka topic which stores websub registered topics. All the hubs must be pointed to the same Kafka topic which stores websub registered topics.
